...
#
#              Another script will drop all existing audit tables. User should run this 
...
die "\nUSAGE: $0 -login <username/password> -db <DB name> -dest <destination of output, path/file.sql> [-tablename <'table1 table2 ...'>] [-storagesizeFactor <rel>]\n" unless ( $sLogin && $sDB && $sDestination );
...
Note: This script is machine-generated, It drops only those audit tables which
      conflict when creating audit tables. Always run the the Perl program which
      generated this script before running this script.
      $sDir/$0
