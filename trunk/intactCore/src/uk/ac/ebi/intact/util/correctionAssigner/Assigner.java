...
public class Assigner {
    ...
    public Assigner() throws Exception, IntactException {
        lister = new ExperimentLister();
     ...
     * description = nameOfTheSuperCurator) and add each experiments to the Collection of experiments to correct of
     * each corresponding SuperCurator.
     *
    ...
    /**
     * Given a percentage and a the total number of experiments to correct, it return the number of pubmed
     * corresponding to the given ratio. As it would make no sence that a SuperCurator would have 1.5 pubmedId to
     * correct, we approximate the result to the inferior integer and return the int (ex : 1.6 will become 1).
     *
     ...
     *      on day 1  CuratorA starts entering the data corresponding to the pubmedId 1 and therefore create exp1 and
     *      exp2.
     *      Therefore on day 1, SuperCuratorB get assigned this pubmedId1 to correct, so an annotation
     *      reviewer(SuperCuratorB) is added to exp1 and exp2.
     *
     *      on day 2, CuratorA finishes entering the data corresponding to the pubmedId 1 in creating exp3. As pubmed1
     *      is already assigned to SuperCuratorB, superCuratorB should automatically be assigned exp3 and exp3 should be
     *      remooved from the list of experiments to be assigned, and exp3 should be added a reviewer annotation. As
     *      this pubmedId was affected the day before it does not count in the number of pubmed the curator should
     ...
     * @param pubmedToExp, map associating the pubmedId to the Collection of corresponding experiments existing in the
     * database and needing to be corrected.
    ...
    /**
     * ex : 2 SuperCurators John and Jane. John and Jane must correct 50% of the total number of pubmed to correct.
     *      3 pubmedw to correct
     *      getNumberOfPubmed(50, 3) will return 1. As it would make no sense to affect a pubmed and a half.
     *      As a consequence, 1 pubmed would be not affected to anybody.
     *
     *      So for each pubmedId remaining we go through the Collection of SuperCurators and if the superCurator to come
     *      is not the one who created the experiment we affect it to him.
     *
     * @param notAssignedPmid2creator, map associating the not assigned pubmedIds to their creator.
     * @param pmid2ExpColl, map associating the pubmedId to the Collection of corresponding experiments existing in the
     * database and needing to be corrected.
     * @param notAssignedExperiments, Collection of not assigned experiments.
     * @throws IntactException
     */
    ...
    /**
     * Once an experiment has been assigned to a SuperCurator we must add a reviewer annotation (with
     * cvTopic = reviewer and description = name of the curator) so that from one day to an another we keep trace in the
     * database of the SuperCurator who has to do the correction. Like that we do not re-affect the same experiment to
     * a different personne everyday.
     *
     * @param expAc
    ...
    public void addReviewerAnnotation(String expAc, String reviewerName) throws Exception {
        ...
        } else throw new Exception("Experiment [" + experiment.getAc() + "," + experiment.getShortLabel() + "] is not persistent");
        helper.closeStore();
    }
    ...
    /**
     * This method should be call after the method treatAssignedExperiments().
     * It take care of affecting all the not assigned experiments to the superCurators.
     * @throws Exception
     */
