...
public class InsertComplexGiot {
    ...
    private HashMap proteins = new HashMap( 7000 );
    ...
    public InsertComplexGiot ( String giotUrl, String cgSptrMappingUrl, IntactHelper helper )
    ...
    public void displayStatistics() {
        System.out.println( "#Giot line processed: " + beanReader.getItemCount() );
    }
...
//                throw e; // pass it to the caller.
    ...
    private void loadCommonObjects() throws IntactException {
        ...
        bioSourceFactory = null;

        // TODO check if it is really an aggregation !
        ...
        if ( giotExperiment == null ){
            ...
            giotExperiment = new Experiment( owner, "giot-2003-ex1", fly );
    ...
    private String getGNLine (String ac) {
...
//        System.out.println ( bean );
         ...
         * Protein 1
         */
        String sptr1 = (String) mapping.get( bean.getCg1() );
        if( sptr1 == null ) {
            throw new IntactException ( "Could not find the mapping corresponding to CG: " + bean.getCg1() );
        }
        ...
        try {
            if (sptr1.indexOf( "-" ) == -1) {
                // protein
                protein1 = (Protein) helper.getObjectByXref( Protein.class, sptr1 );
            } else {
                // splice variant
                protein1 = (Protein) helper.getObjectByLabel( Protein.class, sptr1 );
            }

        } catch (DuplicateLabelException dle) {
            // TODO bug here, can't work

            Collection resultList = null;
            if (sptr1.indexOf( "-" ) == -1) {
                // protein
                resultList = helper.getObjectsByXref( Protein.class, sptr1 );
            } else {
                // splice variant
                resultList = helper.search( Protein.class.getName(), "shortLabel", sptr1);
            }

            System.out.println ( "\nMultiple protein found for label: " + sptr1 );
        ...
        String sptr2 = (String) mapping.get( bean.getCg2() );
        if( sptr2 == null ) {
            throw new IntactException ( "Could not find the mapping corresponding to CG: " + bean.getCg2() );
        }
        ...
        try {
            if (sptr2.indexOf("-") == -1) {
                // protein
                protein2 = (Protein) helper.getObjectByXref( Protein.class, sptr2 );
            } else {
                // splice variant
                protein2 = (Protein) helper.getObjectByLabel( Protein.class, sptr2 );
            }
            protein2 = (Protein) helper.getObjectByLabel( Protein.class, sptr2 );
        } catch ( DuplicateLabelException dle ) {
            Collection resultList = null;
            if (sptr2.indexOf( "-" ) == -1) {
                // protein
                resultList = helper.getObjectsByXref( Protein.class, sptr2 );
            } else {
                // splice variant
                resultList = helper.search( Protein.class.getName(), "shortLabel", sptr2 );
            }
            System.out.println ( "\nMultiple protein found for label: " + sptr2 );
            System.out.println ( "Here they are: " );
            for ( Iterator iterator = resultList.iterator (); iterator.hasNext (); ) {
                Protein protein = (Protein) iterator.next ();
                System.out.println ( protein );
     ...
     * @return
     */
    private String createInteractionShortLabel(String bait, String prey) throws IntactException {
     ...
     */
    private Interaction createInteraction( Protein proteinBait, Protein proteinPrey,
                                           String pBait, String pPrey) throws IntactException {
        ...
        interaction.setKD( new Float( "0.0" ) );
        interaction.setBioSource( fly );
    ...
    public static void main ( String[] args ) throws Exception {
        ...
        if ( args.length < 3 ) {
            // usage
            System.err.println ( "Usage: InsertComplexGiot <Giot source URL> <CG - SPTR mapping filename> <excluded_interaction filename>" );
            System.exit( 1 );
        }
        ...
        // "/homes/skerrien/Flybase2SptrMapping-lt96.HashMap.ser"
