...
/**
 * <p>
 * This bean provides JSPs with the information needed to display a
 * 'partners' view of a search result. This view is defined by the
 * mock web page specifications of June 2004. Typically a user will
 * search for a Protein, and a corresponding 'partners' view of related
 * information will be displayed.
 * </p>
 * <p>
 * This class provides JSPs with as much information as possible in a
 * display-friendly (ie String) format - however for complex beans
 * the JSP itself will need to obtain the information it needs to
 * render the web page.
 * </p>
 * <p>
 * As at June 2004, the summary page needs to show the following information
 * for each Protein found in the search results:
 * <ul>
 * <li>Intact Name</li>
 * <li>Intact Ac</li>
 ...
 * <li>Description</li>
 * </ul>
 * After a row for the primary result, the Interaction partners are lisetd,
 * with the same information.
 * </p>
 * <p>
 * NOTE: If a JSP wishes to display multiple search results then a SummaryViewBean
 * should be created for each result and displayed in turn. This avoids unnecessary
 * complications in the code itself and leaves the display decisions to the
 * JSP, where it belongs.
 * </p>
 *
...
public class PartnersViewBean extends AbstractViewBean {
    ...
    /**
     * Assumed a List of these, as that is what is in the single
     * Protein view. BUT the 'summary' web page only has ONE gene name.
     * NEEDS TO BE CLARIFIED
     */
    private StringBuffer geneNames;

    /**
     * A String representation of the number of Interactions this Protein
     * takes part in.
     */
     ...
     * the context path to the search application, a general searchURL and the help link.
    ...
    /**
     * NB In the overview webpage mockup, this is hyperlinked to
     * search with the shortLabel.
     * @return
     */
    ...
    /**
     * NB in the overview mockup page, this is hyperlinked to provide
    ...
    /**
        ...
        public String getGeneNames() {
            ...
            if(geneNames == null) {
                ...
                if(geneNames.length() > 0) geneNames.deleteCharAt(geneNames.lastIndexOf(","));
    ...
    /**
     * This provides beans of the number of Interactions this Protein
     * participates in.
     * ISSUE: This is a hyperlinked number on the web page mockup, pointing
     * to the 'detail-blue' page mockup. This seems crazy since if eg a Protein has 30
     * Interactions, all in different Experiments, the amount of detail displayed
     * would be unreadable!! DESIGN DECISION: see search link comment later.
     *
    ...
    /**
     * Returns a fully populated URL to perform a search for all this Protein's
     * Interactions. ISSUE: This is used in the mockups to fdisplay DETAIL. This would
     * be unmanageable for large Interaction lists spread across Experiments. DESIGN
     * DECISION: make th link go to the main 'simple' page to list the Interactions
     * *there*, so users can then choose which detail they want.
     * @return String The complete search URL to perform a (possibly multiple) search
     * for this Protein's Interactions
     */
    ...
    /**
     * Provides the beans of the wrapped Protein's interaction partners for display.
     * The beans are in the form of a Set of SummaryViewBeans so that each partner's
     * beans can be displayed in the same way as the wrapped Protein. Sets are used because
     * we don't want to display a set of beans more than once.
     * @return Set a Set of SummaryViewBeans for each interaction partner, or empty if
     * there are none.
     */
    ...
    /**
     * Provides the URL for linking to Uniprot, already filled in with
     * the correct AC from this Protein (ie it has its PrimaryID added in)
            ...
            if(xref != null) {
                ...
                for (Iterator it = annotations.iterator(); it.hasNext();) {
                    ...
                    if (annot.getCvTopic().getShortLabel().equals("search-url")) {
                        ...
                        uniprotURL = SearchReplace.replace(searchUrl, "${ac}", xref.getPrimaryId());
                        break;
                    }
                }
            }
    ...
    /**
     * Provides a String representation of a URL to perform a basic search on
     * this Protein's AC. Thus a general search is performed using this Proterin's AC.
    ...
    /**
     * Have to override this because we accumulate 'partner' viewbeans and
     * even though they are held in a Set, equality based on identity will not be
     * good enough to prevent duplicates being added. Instead we base bean equality
     * on the equality of the wrapped Proteins.
