...
/**
 * This class provides JSP view information for a particular AnnotatedObject. Its main
 * purpose is to provide very simple beans for display in an initial search result page.
 * Currenty the types that may be displayed with this bean are Experiment, Protein,
 * Interaction and CvObject.
 *
...
public class SimpleViewBean extends AbstractViewBean {
    ...
    /**
     * The AnnotatedObject (currently Experiment, Interaction, Protein or
     * CvObject) that the view bean provides beans for.
     */
    private AnnotatedObject obj;

    /**
     * Used for wrapped Experiments and Interactions. In the case of
     * Experiments this holds the number of Interactions it has, but for
     * Interactions it contains the number of Proteins for the Interaction.
     * In other cases (currently CvObject and Protein) this should be an empty
     * String (JSPs hate null objects :-)). It will be initialised to that in
     * the constructor.
     */
     ...
     * (eg a wrapped ProteinImpl will have the intact type of 'Protein'). Would
     ...
     * can be any one of Experiment, Protein, Interaction or CvObject type.
     ...
     * this AnnotatedObject's beans (curently via AC)
     ...
     * AnnotatedObject. For an Experiment this will be the number of Interactions it
     * has; for an Interaction it will be the number of active instances (Proteins) it has.
     * In all other cases this method will return an empty String.
     * NOTE: For Interactions this will return the number of Components it has - this
     * may need to become more sophisticated when Components have Interactors other than
     * single Proteins.
    ...
    /**
     * Provides the basic Intact type of the wrapped AnnotatedObject (ie no java
     ...
     * in objects of type XXXImpl. For subclasses of CvObject we only need 'CvObject'
    ...
    public String getIntactType() {
        ...
        if(intactType == null) {
            //set on first call
            if(obj instanceof CvObject) intactType = "CvObject";
            else {
    ...
    /**
     * This method will count up the number of Proteins that a List of
     * Components contains. It will recurse through all nested Interactions if
     * necessary, and so handle complexes.
