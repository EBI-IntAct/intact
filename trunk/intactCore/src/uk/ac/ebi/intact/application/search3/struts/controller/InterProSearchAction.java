...
/**
 * This Action class performs the determination of UniprotKB IDs out of IntAct ACs.
 * <br>
 * These UniprotKB IDs are then used to build up dynamically a link to the corresponding entries in the
 * InterPro Database, like <br><b>http://www.ebi.ac.uk/interpro/ISpy?ac=Q9UKX3%2CQ9UK45%2CP62312</b><br>
 * where <i>Q9UKX3</i>, <i>Q9UK45</i> and <i>P62312</i> specify the UniprotKB IDs related to every IntAct-Protein.
 * <p/>
 * Dependent on the user's selection, one has to distinguish from 3 cases:<br>
 * <ol>
 * <li> all of the selected Proteins do have an UniProtKB ID, so it will be forwarded to the <i>Waiting Page</i>
 * <li> no Protein at all does have an UniProtKB ID, so it will be forwarded to the general <i>Error Page</i>
 * <li> at least one Protein do have an UniProtKB ID and at least one Protein has none, so it will be forwarded
 * to the <i>Waiting Page</i> as well, but including one ore more warning hints.
 * </ol>
 * <p/>
...
public class InterProSearchAction extends AbstractResultAction {
    ...
    /**
     * Iterates through a Set of unmapped Proteins (meaning: no corresponding UniprotKB ID was found) and
     * appends the Protein-shortlabel and Protein-AC to a StringBuffer afterwards.
     *
     * @param unmappedProteins contains those Proteins, where no UniprotKB ID exists.
     * @return proteinsWithoutUniprotidBuffer a <code>String</code> containing the shortlabel and proteinAC.
     *         of those Proteins where no uniprotKB ID is available
     *         (e.g.: myome-afcs_mouse (EBI-659248), fnlb-afcs_mouse (EBI-688023)...)
     */
    ...
    /**
     * Builds the html-View for that part of the message where an UniprotKB ID could be found for every Intact-AC
     * and no error/warning occured.
     *
     * @param htmlBuffer      used to append all Html-statements that will finally create the Summary-Message.
     * @param intactACsLength total number of intact ACs.
    ...
    /**
     * Builds the html-View for that part of the message where the Maximum Number of selectable
    ...
    /**
     * Parses a <code>String</code>, which might include doubled entries, into a <code>HashSet</code>.
     * By using a HashSet, it is guaranteed that every entry in it is unique.
     *
