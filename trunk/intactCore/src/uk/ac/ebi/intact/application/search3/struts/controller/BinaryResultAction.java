...
public class BinaryResultAction extends AbstractResultAction {
    ...
    protected String processResults(HttpServletRequest request, String helpLink) {
        ...
        if (Protein.class.isAssignableFrom(results.iterator().next().getClass())) {
            ...
            for (Iterator it = results.iterator(); it.hasNext();) {
                beanList.add(new PartnersViewBean((Protein)it.next(), helpLink, searchURL, request.getContextPath()));
            }
    ...
    /**
        ...
        protected AbstractViewBean getAbstractViewBean ( Object result, IntactUserIF user, String contextPath ) {
            ...
            return new PartnersViewBean((Protein)result, user.getHelpLink(), searchURL, contextPath);

            //previous code (returns a bean that wraps a whole Collection).....
            logger.info( "binary action: building view beans..." );
            if(Collection.class.isAssignableFrom(result.getClass()))
            return ViewBeanFactory.getInstance().getBinaryViewBean ( (Collection)result, user.getHelpLink(), contextPath );
            return null;
        }
