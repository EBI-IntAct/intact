 ...
 *
 * TODO: This class needs refactoring/removing when all new views are in place - it
 * may be possible to integrate the new 'simple' view into here, but currently it needs
 * to generate a LIST of viewbeans, not a SINGLE one (but maybe see partners view as
 * this is similar but is included here in the 'hacked' code below!)
 *
 * @author IntAct Team
 * @version $Id$
 */
public abstract class AbstractResultAction extends IntactBaseAction {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        if ((Experiment.class.isAssignableFrom(resultType)) || (Interaction.class.isAssignableFrom(resultType))) {
            //use the procesRequest method instead....
            return mapping.findForward(processResults(request, user.getHelpLink()));
        }
    ...
    /**
     * Builds a view bean depending upon the search result data (eg Experiment, single
     * Protein, etc). Specific subclasses should have a means of creating the appropriate
     * view bean to be forwarded.
     ...
     * links.
     * @return AbstractViewBean a view bean appropriate for the display item(s), or null
     * if there was a problem with either the parameters or in building a bean
     */
    protected abstract AbstractViewBean getAbstractViewBean (Object result, IntactUserIF user, String contextPath );
