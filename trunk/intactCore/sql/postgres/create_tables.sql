...
(     ac                      VARCHAR  (30)    NOT NULL
    ...
    , owner_ac                VARCHAR  (30)    CONSTRAINT fk_Institution_owner REFERENCES Institution(ac)
...
(       ac                      VARCHAR (30)    NOT NULL
     ...
     ,  timestamp               TIMESTAMP       DEFAULT  now()   NOT NULL
     ,  userstamp               VARCHAR (30)    DEFAULT  USER    NOT NULL
     ,  owner_ac                VARCHAR (30)    CONSTRAINT fk_ControlledVocab_owner REFERENCES Institution(ac)
...
;
CREATE UNIQUE INDEX uq_CVocab_objClass_ShortLabel ON ControlledVocab(objClass,shortLabel) 
;
...
(         ac                    VARCHAR (30)    NOT NULL
        ...
        , crc64                 VARCHAR (16)
        , formOf                VARCHAR (30)    CONSTRAINT fk_Interactor_formOf REFERENCES Interactor(ac)
        , proteinForm_ac        VARCHAR (30)    CONSTRAINT fk_Interactor_proteinForm_ac REFERENCES ControlledVocab(ac)
        /* Colums belonging to Interactor */
        , objClass              VARCHAR (255)   
        , bioSource_ac          VARCHAR (30)    CONSTRAINT fk_Interactor_bioSource REFERENCES BioSource(ac)
        , interactionType_ac    VARCHAR (30)    CONSTRAINT fk_Interactor_interactionType REFERENCES ControlledVocab(ac)
...
(         ac                      VARCHAR (30)    NOT NULL
        ...
        , timestamp               TIMESTAMP       DEFAULT  now()   NOT NULL
        , userstamp               VARCHAR (30)    DEFAULT  USER    NOT NULL 
        , interactor_ac           VARCHAR (30)    CONSTRAINT fk_Component_interactor REFERENCES Interactor(ac)  ON DELETE CASCADE
        , interaction_ac          VARCHAR (30)    CONSTRAINT fk_Component_interaction REFERENCES Interactor(ac) ON DELETE CASCADE
        , role                    VARCHAR (30)    CONSTRAINT fk_Component_role REFERENCES ControlledVocab(ac)   
        , expressedIn_ac          VARCHAR (30)    CONSTRAINT fk_Component_expressedIn REFERENCES BioSource(ac)
        , owner_ac                VARCHAR (30)    CONSTRAINT fk_Component_owner REFERENCES Institution(ac)
...
(         ac                      VARCHAR (30)    NOT NULL
        ...
        , timestamp               TIMESTAMP       DEFAULT  now()   NOT NULL
        , userstamp               VARCHAR (30)    DEFAULT  USER    NOT NULL
        , topic_ac                VARCHAR (30)    CONSTRAINT fk_Annotation_topic REFERENCES ControlledVocab(ac)
        , owner_ac                VARCHAR (30)    CONSTRAINT fk_Annotation_owner REFERENCES Institution(ac)
...
(       ac                      VARCHAR (30)    NOT NULL
      ...
      , timestamp               TIMESTAMP       DEFAULT  now()   NOT NULL
      , userstamp               VARCHAR (30)    DEFAULT  USER    NOT NULL
      , bioSource_ac            VARCHAR (30)    CONSTRAINT fk_Experiment_bioSource REFERENCES BioSource(ac)
      , detectMethod_ac         VARCHAR (30)    CONSTRAINT fk_Experiment_detectMethod REFERENCES ControlledVocab(ac)
      , identMethod_ac          VARCHAR (30)    CONSTRAINT fk_Experiment_identMethod REFERENCES ControlledVocab(ac)
      , relatedExperiment_ac    VARCHAR (30)    CONSTRAINT fk_Experiment_relatedExp REFERENCES Experiment(ac)
      , owner_ac                VARCHAR (30)    CONSTRAINT fk_Experiment_owner REFERENCES Institution(ac)
      , shortLabel              VARCHAR (20)
      , fullName                VARCHAR (250) 
)
;
...
(       ac                      VARCHAR (30)    NOT NULL
     ...
     ,  timestamp               TIMESTAMP       DEFAULT  now()   NOT NULL
     ,  userstamp               VARCHAR (30)    DEFAULT  USER    NOT NULL
     ,  qualifier_ac            VARCHAR (30)    CONSTRAINT fk_Xref_qualifier REFERENCES ControlledVocab(ac)
     ,  database_ac             VARCHAR (30)    CONSTRAINT fk_Xref_database  REFERENCES ControlledVocab(ac)
     ,  parent_ac               VARCHAR (30)    -- eh missing constraint here??
     ,  owner_ac                VARCHAR (30)    CONSTRAINT fk_Xref_owner REFERENCES Institution(ac)
...
(       ac                          VARCHAR (30) NOT NULL
      ...
      , owner_ac                    VARCHAR (30)   CONSTRAINT fk_IntactNode_owner REFERENCES Institution(ac)
...
(       interaction_ac          VARCHAR (30)    NOT NULL   CONSTRAINT fk_Int2Exp_interaction REFERENCES Interactor(ac)  ON DELETE CASCADE
      , experiment_ac           VARCHAR (30)    NOT NULL   CONSTRAINT fk_Int2Exp_experiment  REFERENCES Experiment(ac)  ON DELETE CASCADE
      , deprecated              DECIMAL(1)      DEFAULT    0       NOT NULL
      , created                 TIMESTAMP       DEFAULT    now()   NOT NULL
      , userstamp               VARCHAR (30)    DEFAULT    USER    NOT NULL
      , updated                 TIMESTAMP       DEFAULT    now()   NOT NULL
...
(       cvobject_ac             VARCHAR (30)    NOT NULL CONSTRAINT fk_cvobj2Annot_cvobject   REFERENCES ControlledVocab(ac) ON DELETE CASCADE
     ,  annotation_ac           VARCHAR (30)    NOT NULL CONSTRAINT fk_cvobj2Annot_annotation REFERENCES Annotation(ac) ON DELETE CASCADE
     ,  deprecated              DECIMAL(1)      DEFAULT  0       NOT NULL
     ,  created                 TIMESTAMP       DEFAULT  now()   NOT NULL
     ,  userstamp               VARCHAR (30)    DEFAULT  USER    NOT NULL  
     ,  updated                 TIMESTAMP       DEFAULT  now()   NOT NULL
...
(       biosource_ac            VARCHAR (30)    NOT NULL CONSTRAINT fk_bio2Annot_biosource   REFERENCES Biosource(ac)  ON DELETE CASCADE
     ,  annotation_ac           VARCHAR (30)    NOT NULL CONSTRAINT fk_bio2Annot_annotation  REFERENCES Annotation(ac) ON DELETE CASCADE
     ,  deprecated              DECIMAL(1)      DEFAULT  0       NOT NULL
     ,  created                 TIMESTAMP       DEFAULT  now()   NOT NULL
     ,  userstamp               VARCHAR (30)    DEFAULT  USER    NOT NULL  
     ,  updated                 TIMESTAMP       DEFAULT  now()   NOT NULL
...
CREATE TABLE Cv2Cv
(
       parent_ac               VARCHAR(30)      NOT NULL CONSTRAINT fk_Cv2Cv_parent REFERENCES ControlledVocab(ac) ON DELETE CASCADE
    ,  child_ac                VARCHAR(30)      NOT NULL CONSTRAINT fk_Cv2Cv_child REFERENCES ControlledVocab(ac) ON DELETE CASCADE
    ,  deprecated              DECIMAL(1)       DEFAULT  0       NOT NULL
    ,  created                 TIMESTAMP        DEFAULT  now()   NOT NULL
    ,  updated                 TIMESTAMP        DEFAULT  now()   NOT NULL
    ,  timestamp               TIMESTAMP        DEFAULT  now()   NOT NULL
