...
public class BinaryInteractionSet {
    ...
    public BinaryInteractionSet(String path) throws IOException {
        ...
        while ((line = in.readLine()) != null) {
            ...
            if (totalProts <= 1) continue; // ignore empty lines & proteins with no listed partners
            prot1 = items[0];
            prot2 = items[1];
            ProteinPair bi = new ProteinPair(prot1, prot2);
            if (verbose) System.out.println(bi.toString() + " added to binary interaction set.");
            biSet.add(bi);
        }
        if (verbose) System.out.println(biSet.size() + " binary interactions found.");
        in.close();
    }
    ...
    public HashSet<String> getInteractionPartners(String prot) {
        ...
        for (ProteinPair pair : biSet) {
            names = pair.getNames();
            if (names[0].equals(prot)) partners.add(names[1]);
            else if (names[1].equals(prot)) partners.add(names[0]);
        }
        return partners;
    ...
    public BinaryInteractionSet(Collection<ProteinPair> collection) {
        biSet = new HashSet<ProteinPair>();
        for (ProteinPair pair : collection) biSet.add(pair);
    }
