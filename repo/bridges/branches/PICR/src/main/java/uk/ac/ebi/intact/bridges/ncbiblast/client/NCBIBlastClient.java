...
public class NCBIBlastClient {
    ...
    public byte[] poll(String jobid) throws NCBIBlastClientException{
        ...
        for (WsResultType resultType : resultTypes) {
            // Get the results
            if (resultType.getIdentifier().equals(resultFormat)) {
                byte[] resultbytes = null;
                try {
                    resultbytes = this.service.getResult(jobid, resultType.getIdentifier(), null);
                ...
                if (resultbytes == null) {
                    System.err.println("Null result for " + resultType.getIdentifier() + "!");
                }
    ...
    public WsParameterValues getDatabases() {
        WsParameterDetails details = this.service.getParameterDetails("database");
    ...
    //Overloaded to accept input params
   public Job blastSequence(String email, String sequence, InputParameters params, String ... databases) throws NCBIBlastClientException {
        ...
        //Job job = null; // not sure why this is here
        Job job = new Job(runNCBIBlast(email, params), sequence);
        checkStatus(job);
        return job;
    }
