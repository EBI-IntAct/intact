...
public class IntactContext implements Serializable, Closeable {
    ...
    public static void initContext( DataConfig defaultDataConfig, IntactSession session ) {
        ...
        // init Spring
        ClassPathXmlApplicationContext springContext = new ClassPathXmlApplicationContext(new String[] {"/intact.spring.xml", "/META-INF/standalone/jpa-standalone.spring.xml"});

        //RuntimeConfig.initRuntime( session, defaultDataConfig );
        RuntimeConfig runtimeConfig = (RuntimeConfig) springContext.getBean("runtimeConfig");
        IntactConfigurator configurator = (IntactConfigurator) springContext.getBean("intactConfigurator");

        runtimeConfig.addDataConfig(defaultDataConfig);
        configurator.initIntact( session );
    ...
    public UserContext getUserContext() {
        return UserContext.getCurrentInstance( runtimeConfig, session );
    }
