create or replace FUNCTION proc_split_alias() RETURNS VOID AS $$  -- cursor over all Aliases
DECLARE alias_cur CURSOR FOR SELECT * FROM ia_alias;
            ...
            ELSE
              ...
              ELSE
                ...
                IF(v_count <> 0) THEN
                    ...
                    -- copy Alias into the right table
                    INSERT INTO ia_component_alias (
                        AC,
                        DEPRECATED,
                        CREATED,
                        UPDATED,
                ...
                ELSE
                  ...
                  raise debug 'Deleting orphan Alias...';
                  -- DBMS_OUTPUT.PUT_LINE('DELETE FROM ia_alias WHERE ac = ''v_alias_ac''');
                  raise debug 'DELETE FROM ia_alias WHERE ac = \'%\'', v_alias_rec.AC;
				END IF;  -- component
                END IF;  -- publications
              END IF;  -- feature
            END IF;  -- biosource
