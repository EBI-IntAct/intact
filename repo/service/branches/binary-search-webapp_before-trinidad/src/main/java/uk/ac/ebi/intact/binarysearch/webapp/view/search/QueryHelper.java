...
public class QueryHelper {
    ...
    public static String createQuery(AdvancedSearch search, List<Term> interactionTypeTerms, List<Term> detectionMethodTerms) {
        ...
        if (isValidValue(search.getPubId())) {
            ...
            sb.append(concatFieldAndValue(docDef.getColumnDefinition(MitabDocumentDefinition.PUB_ID).getShortName(), search.getPubId()));
        }
        if (isValidValue(search.getPubFirstAuthor())) {
            sb.appendOperand(search);
            sb.append(concatFieldAndValue(docDef.getColumnDefinition(MitabDocumentDefinition.PUB_AUTH).getShortName(), search.getPubFirstAuthor()));
        }
        if (isValidValue(search.getInteractionId())) {
            sb.appendOperand(search);
            sb.append(concatFieldAndValue(docDef.getColumnDefinition(MitabDocumentDefinition.INTERACTION_ID).getShortName(), search.getInteractionId()));
        }
        ...
        if (isValidValue(search.getInteractionType())) {
            ...
            if (interactionTypeTerm != null) {
                sb.append(concatFieldAndTerm(docDef.getColumnDefinition(MitabDocumentDefinition.INT_TYPE).getShortName(), interactionTypeTerm, search.isIncludeInteractionTypeChildren()));
            } else {
                sb.append(concatFieldAndValue(docDef.getColumnDefinition(MitabDocumentDefinition.INT_TYPE).getShortName(), search.getInteractionType()));
        ...
        if (isValidValue(search.getDetectionMethod())) {
            ...
            if (detectionMethodTerm != null) {
                sb.append(concatFieldAndTerm(docDef.getColumnDefinition(MitabDocumentDefinition.INT_DET_METHOD).getShortName(), detectionMethodTerm, search.isIncludeDetectionMethodChildren()));
            } else {
                sb.append(concatFieldAndValue(docDef.getColumnDefinition(MitabDocumentDefinition.INT_DET_METHOD).getShortName(), search.getDetectionMethod()));
            }
        }
