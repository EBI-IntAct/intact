...
ParticipantDrawer = function(interactionInformation){
    ...
    this.drawLegend = function(x, y){
        ...
        var w = fTypesText.getBBox().width;
        if(w == 0){
            w = this.getWidthOfTextNode("feature types")/2;
        }
		var line = this._shapeDrawer.getLine(x + textGap, yLine, x + textGap + w, yLine);
		line.attr("stroke-width",0.5);
    ...
    this.calculateLegendPictureWidth = function(legendItemWidth, textGap){
        ...
        for (var category in this._interactionInformation._typeCategories) {
            ...
            var w = 0;
            if(bb.width == 0){
                w = this.getWidthOfTextNode(category)/2;
            }else{
                w = bb.width;
            }
            if(w > width){
    ...
    this.calculateLegendRangetypeSectionWidth = function(textGap){
        ...
        for (var functionName in this._calledFunctions) {
            $(this._calledFunctions[functionName]).each(function(){
                ...
                var w = 0;
                if(bb.width == 0){
                    w = self.getWidthOfTextNode(text);
                }else{
                    w = bb.width;
                }
    ...
    this.getWidthOfTextNode = function(text){
        return this._interactionInformation._utils.getTextDimension(text, this._interactionInformation._myElement).width;
    };
    ...
    this.drawLegendRangetypeSection = function(itemX, y, yText, yLine, legendItemWidth, legendHeight, textGap, gap){
		...
		if (!this._interactionInformation._utils.isEmptyObject(this._calledFunctions)) {
            ...
            var w = rTypesText.getBBox().width;
            if(w == 0){
                w = this.getWidthOfTextNode("range types")/2;
            }
			var line2 = this._shapeDrawer.getLine(itemX, yLine, itemX + w, yLine);
