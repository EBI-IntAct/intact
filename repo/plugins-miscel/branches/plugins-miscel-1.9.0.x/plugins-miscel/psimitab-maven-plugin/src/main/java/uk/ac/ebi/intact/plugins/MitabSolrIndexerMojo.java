...
public class MitabSolrIndexerMojo extends IntactAbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try {
            if ( recreateOntologyData ) {
                ...
                indexer.indexObo(
                        ontologyMappings.toArray(
                                new uk.ac.ebi.intact.plugins.OntologyMapping[ontologyMappings.size( )]
                        )
                ) ;
            }
        } catch ( Exception e ) {
        ...
        for ( int i = 0; i < availableProcessors; i++ ) {
            ...
            writeLog( "Starting thread '" + threadName + "' to process lines "+ firstLine +".."+ (firstLine + batchSize)
                      + " (batchSize=" + batchSize +") ..." );

            final IndexerWorker worker = new IndexerWorker( mitabFile,
                                                            hasMitabHeader,
                                                            solrServerUrl,
                                                            ontologySolrServerUrl,
                                                            firstLine,
                                                            batchSize );
            workers.add( worker );
    ...
    private static class IndexerWorker implements Runnable {
        ...
        public IndexerWorker( URL mitabFile, boolean hasHeader, String solrServerUrl,
                              String ontologyServerUrl, int firstLine, int batchSize ) {
        ...
        public void run() {
            ...
            try {
                ...
                e.printStackTrace(  );
                throw new RuntimeException( name + " - Problem processing mitab file - firstLine=" + firstLine +
                                            " - batchSize=" + batchSize, e );
            }
        }
    }
