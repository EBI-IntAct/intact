...
public class MutationAnalysisFeature extends Rule<Feature> {

    public Collection<GeneralMessage> check( Feature feature ) throws SanityRuleException {
        Collection<GeneralMessage> messages = new ArrayList<GeneralMessage>();

        if ( !isIgnored( feature, MessageDefinition.FEATURE_DELETION_FEATURE_TOO_LONG ) ) {
            if ( feature.getCvFeatureType() == null ) {
                return messages;
            }

            String cvFeatureTypeIdentityMi = feature.getCvFeatureType().getIdentifier();
