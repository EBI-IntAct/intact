 ...
 * @since TODO
 */
public class EnzymeMock {
    public static CvComponentRole getMock(){
        CvComponentRole enzyme = CvObjectMock.getMock(CvComponentRole.class,CvComponentRole.ENZYME, CvComponentRole.ENZYME);

        enzyme = (CvComponentRole) IntactObjectSetter.setBasicObject(enzyme);

        CvObjectXref xref = XrefMock.getMock(CvObjectXref.class, PsiMiMock.getMock(), IdentityMock.getMock(),CvComponentRole.ENZYME_PSI_REF);
        enzyme.addXref(xref);
